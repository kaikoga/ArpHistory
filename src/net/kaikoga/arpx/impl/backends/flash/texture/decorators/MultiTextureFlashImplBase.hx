package net.kaikoga.arpx.impl.backends.flash.texture.decorators;
	...
	override public function getFaceIndex(params:IArpParamsRead = null):Int {
		...
		if (face != null) {
			if (this.indexesByFaces.exists(face)) {
				...
				this.texture.arpDomain.log("texture", 'MultiTextureFlashImplBase.getFaceIndex(): Face name not found in: ${this.texture.arpSlot}:$params');
			}
		}
		...
		try {
			...
			this.texture.arpDomain.log("texture", 'MultiTextureFlashImplBase.getFaceIndex(): Illegal index: ${this.texture.arpSlot}:$params');
		}
