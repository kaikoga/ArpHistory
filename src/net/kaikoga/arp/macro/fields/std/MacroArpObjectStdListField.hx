...
class MacroArpObjectStdListField extends MacroArpObjectField {
	...
	override public function buildConsumeSeedElementBlock(cases:Array<Case>):Void {
		...
		switch (this.type) {
			case MacroArpObjectValueType.PrimInt:
				caseBlock.push(macro @:pos(this.nativePos) { this.$iFieldName.add(Std.parseInt(element.value())); });
			case MacroArpObjectValueType.PrimFloat:
				caseBlock.push(macro @:pos(this.nativePos) { this.$iFieldName.add(Std.parseFloat(element.value())); });
			case MacroArpObjectValueType.PrimBool:
				caseBlock.push(macro @:pos(this.nativePos) { this.$iFieldName.add(element.value() == "true"); });
			case MacroArpObjectValueType.PrimString:
				caseBlock.push(macro @:pos(this.nativePos) { this.$iFieldName.add(element.value()); });
		}
	}
