...
class MacroArpObjectMapField extends MacroArpObjectCollectionFieldBase implements IMacroArpField {
	...
	public function buildHeatLaterBlock(heatLaterBlock:Array<Expr>):Void {
		...
		heatLaterBlock.push(macro @:pos(this.nativePos) { for (slot in this.$iNativeName.slotMap) this._arpDomain.heatLater(slot, $v{arpBarrierRequired}); });
	}
	...
	public function buildDisposeBlock(disposeBlock:Array<Expr>):Void {
		disposeBlock.push(macro @:pos(this.nativePos) { for (slot in this.$iNativeName.slotMap) slot.delReference(); });
	}
