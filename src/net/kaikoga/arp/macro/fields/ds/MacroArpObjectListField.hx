...
class MacroArpObjectListField extends MacroArpObjectCollectionFieldBase implements IMacroArpField {
	...
	public function buildHeatLaterBlock(heatLaterBlock:Array<Expr>):Void {
		if (this.arpBarrier) {
			heatLaterBlock.push(macro @:pos(this.nativePos) { for (slot in this.$iNativeName.slotList) this._arpDomain.heatLater(slot); });
		}
	}
	...
	public function buildConsumeSeedElementBlock(cases:Array<Case>):Void {
		...
		caseBlock.push(macro @:pos(this.nativePos) { this.$iNativeName.slotList.push(this._arpDomain.loadSeed(element, ${this.eArpType}).addReference()); });
	}
