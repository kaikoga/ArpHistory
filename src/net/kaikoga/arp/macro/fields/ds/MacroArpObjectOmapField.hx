...
class MacroArpObjectOmapField extends MacroArpObjectCollectionFieldBase implements IMacroArpField {
	...
	public function buildHeatLaterBlock(heatLaterBlock:Array<Expr>):Void {
		...
		heatLaterBlock.push(macro @:pos(this.nativePos) { for (slot in this.$iNativeName.slotOmap) this._arpDomain.heatLater(slot, $v{arpBarrierRequired}); });
	}
	...
	public function buildDisposeBlock(disposeBlock:Array<Expr>):Void {
		disposeBlock.push(macro @:pos(this.nativePos) { for (slot in this.$iNativeName.slotOmap) slot.delReference(); });
	}
	...
	public function buildConsumeSeedElementBlock(cases:Array<Case>):Void {
		...
		caseBlock.push(macro @:pos(this.nativePos) { this.$iNativeName.slotOmap.addPair(element.key, this._arpDomain.loadSeed(element, ${this.eArpType}).addReference()); });
	}
